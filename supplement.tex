...
\subsection{Old Time Series and Fits}

These sinusoidal fits had very tight priors on the rotation period (3.0 to 3.2 hr) and (3.8 to 4.0 hr), not quite consistent with \citep[][4.2hr]{2015ApJ...799..154M}.
\begin{figure*}[!t]
\centering
\subfloat[2MASS J0835]{
	\includegraphics[width=0.5\textwidth]{images/tser/2mass_0835_t_series.pdf}
	\label{fig:tserPhase0835}
	}
\subfloat[2MASS J1821]{
	\includegraphics[width=0.5\textwidth]{images/tser/2mass_1821_t_series.pdf}
	\label{fig:tserPhase1821}
	}
...
\centering
\subfloat[2MASS J0835]{
	\includegraphics[width=0.5\textwidth]{images/old_tight_period_constraints/amp_vs_wavl_j0835.pdf}
	\label{fig:ampspec0835}
	}
\subfloat[2MASS J1821]{
	\includegraphics[width=0.5\textwidth]{images/old_tight_period_constraints/amp_vs_wavl_j1821.pdf}
	\label{fig:ampspec1821}
	}
	\caption{Sine fit amplitude versus wavelength for each brown dwarf. \sha\ shows no clear trend in wavelength, while \shb\ has a strong slope indicative of Mie-scattering particles.
	 ...
	 \shb\ is shown with the best fit median particle size of 0.17 $\mu$m grains.}
	\label{fig:ampSpec}
\end{figure*} 
...
\section{Spot Model Fit}\label{sec:spotModel}
...
\end{equation}
where $\alpha_1$ and $\alpha_2$ are the areal fractions of the first temperature ($T_1$) for the first and second hemispheres respectively, $B_\lambda$ is the Planck function, and $T_1$ and $T_2$ are the temperatures of the two regions.
...
The $\alpha$ values are both $<<1$, indicating that the model fits with bright patches on a small fraction of the surface.
The maximum to minimum contrast in this spot model is sensitive to the relative areas of these bright patches so a small difference in areas controls the maximum over minimum flux.
...
\section{Re-visiting The $\Delta$ T models}

After Theodora's questions, I decided to re-visit the spot models. I went back to Equation \ref{eq:twoTempAmp} and just plotted some example models as a function of different parameters. 

\begin{figure*}[!t]
\centering
\subfloat[T$_1$ Parameter]{
	\includegraphics[width=0.5\textwidth]{images/two_temp_models/temp_varied.pdf}
	\label{fig:twoTempModelTvar}
	}
\subfloat[$\beta$ Parameter]{
	\includegraphics[width=0.5\textwidth]{images/two_temp_models/beta_varied.pdf}
	\label{fig:twoTempModelBetavar}
	}\\
\subfloat[$\alpha_1$ Parameter]{
	\includegraphics[width=0.5\textwidth]{images/two_temp_models/alpha_1_varied.pdf}
	\label{fig:twoTempModelAlphavar}
	}
	\caption{Variations in model parameters compared to the data, with points near the expected spot coverages and temperature contrasts.}
	\label{fig:twoTempModelParamsVar}
	\vspace{0.1in}
\end{figure*} 
...
\centering
\subfloat[T$_1$ Parameter]{
	\includegraphics[width=0.5\textwidth]{images/two_temp_models/extreme_temp_varied.pdf}
	\label{fig:twoTempModelTvarE}
	}
\subfloat[$\beta$ Parameter]{
	\includegraphics[width=0.5\textwidth]{images/two_temp_models/extreme_beta_varied.pdf}
	\label{fig:twoTempModelBetavarE}
	}\\
\subfloat[$\alpha_1$ Parameter]{
	\includegraphics[width=0.5\textwidth]{images/two_temp_models/extreme_alpha_1_varied.pdf}
	\label{fig:twoTempModelAlphavarE}
	}
	\caption{Variations in model parameters compared to the data, with more extreme parameters that best-fit the data.}
	\label{fig:twoTempModelParamsVarE}
	\vspace{0.1in}
\end{figure*} 

\clearpage
\section{Optical Depth of Possible Haze Clouds}

\subsection{Hemispheric Coverage Assumption}\label{sec:hemisphOpticalDepth}
Let's assume that there is a haze layer that covers 50\% of \shb's surface (ie. one hemisphere) and that the maximum and minimum fluxes represent when this cloudy hemisphere is facing towards or away from our observed line of sight.
This will give us a lower limit on the optical depth to explain the variability amplitudes.
The flux after extinction is 
\begin{equation}
F_\mathrm{obs} = F_\mathrm{d} e^{-\tau}
\end{equation}
where $F_\mathrm{obs}$ is the observed flux, $F_\mathrm{d} $ is the flux from a deep layer underneath the haze and $\tau$ is the optical depth of the haze.
Under our assumption that the haze covers one hemisphere, the brighter hemisphere will have a flux $F_\mathrm{obs}  = F_\mathrm{d}$ so the maximum to minimum flux ratio is
\begin{equation}
f_R = e^{\tau}
\end{equation}
or
...
\tau = \ln{(f_R)}
\end{equation}
From the main paper, the relationship between variability amplitude and $f_R$ is
...
so
\begin{equation}
\tau = \ln{\left(\frac{A+1}{1 - A}\right)}
\end{equation}

\begin{figure}
\begin{centering}
\includegraphics[width=0.5\textwidth]{images/optical_depth/tau_plot_lower.pdf}
\caption{Optical depth if the haze layer is assumed to cover a full hemisphere to explain the flux variations.}\label{fig:opticalDepthLower}
\end{centering}
\end{figure}

\clearpage
\subsection{Optical Depth for Assumed Spot Contrast}

While section \ref{sec:hemisphOpticalDepth} represents a lower limit estimate for the optical depth needed to explain the variability amplitudes, smaller spots/clouds will produce higher contrasts and therefore will need higher optical depths.
In this section, we also include an estimate based on the spot coverage maps.
The {\it Aeolus} code \citep{2015ApJ...814...65K} finds a near-constant spot brightness contrast for all wavelengths of around 50 K.
We start by calculating the flux constrast for a brightness temperature difference of 50 K in the opt panel of Figure \ref{fig:opticalDepthAeolusSpot}.

If we assume that this spot contrast is due to extinction by dust, then the optical depth is, as before $\tau = \ln{(f_R)}$.
The resulting $\tau$ is plotted in the bottom panel of \ref{fig:opticalDepthAeolusSpot}.
We can calculate the column density of the dust if we assume a particle radius.
As found in the paper the radius of the particles $r_p$ is $\sim$ 0.3 $\mu$m.
Then, the column density is:
\begin{equation}
N_{dust} = \frac{\tau} {\pi * r_p^2 Q_{ext}}.
\end{equation}
Assuming $Q_{ext} \sim 1$, then $N_{dust} = 1.2 \times 10^{12}$~m$^{-2}$.
If we assume a density of 0.2 g cm$^{-3}$, this corresponds to a mass column density of 2.7 $\times 10^{-5}$ kg m$^{-2}$.

\begin{figure}
\begin{centering}
\includegraphics[width=0.5\textwidth]{images/optical_depth/tau_plot_spot_constrast.pdf}
\caption{{\it Top Panel} The flux contrast for a 50 K brightness temperature difference. {\it Bottom Panel} Optical depth of a haze layer if this 50 K spot contrast is due to dust extinction.}\label{fig:opticalDepthAeolusSpot}
\end{centering}
\end{figure}
