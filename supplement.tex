...
\clearpage{Airmass Correlation}
One thing to check is that the U shaped light curves near the telluric water bands are correlated to airmass. As I start, I used my code to look at the residuals of the single sinusoid model simultaneously with airmass.

\begin{figure}
\begin{centering}
\includegraphics[width=0.5\textwidth]{images/airmass/residuals_1p38_airmass.pdf}
\caption{Residuals of a sinusoidal fit (with hard priors to ensure fitting out the BD modulation). The magnitude of the residuals is about 5$\times$ what it should be, likely an error in the code.}\label{fig:residAirmass1p38on1821}
\end{centering}
\end{figure}
