...
\colhead{Refs} \\
 &  \colhead{Opt} & \colhead{NIR} &  \colhead{(pc)} & &  & &  \colhead{(K)} & \colhead{(hr)} & \colhead{(km/s)} & \colhead{($\degr$)} \\
 }
\startdata
\sha\ & L5 & L7 & 8.52$\pm$0.08 & 11.94$\pm$0.02 & 2.03$\pm$0.03 & $-$4.05$\pm$0.08 & 1754$\pm$112 & 3.1\tablenotemark{b} & 14.18$\pm$0.43 & 21 & 1-5 \\
\shb\ & L4.5p & L5 & 9.38$\pm$0.03 & 12.40$\pm$0.02 & 1.78$\pm$0.03 & $-$4.18$\pm$0.03 & 1635$\pm$66 & 4.2$\pm$0.1 & 28.85$\pm$0.16 & 76 & 5-11 \\
\enddata
\tablenotetext{a}{Estimated inclination angle $i$ assuming a radius = 1 Jupiter radius, so that $\sin{i}$ = 8.0$\times$10$^{-3}Pv\sin{i}$, where $P$ is in hours and $v\sin{i}$ is in km/s. A value of $i$ $\approx$ 0{\degree} is viewed pole-on, $i$ $\approx$ 90{\degree} is viewed equator-on.}
...
(8) \cite{2016ApJS..225...10F};
(9) \citet{2015ApJ...798...73G};
(10) \citet{2015ApJ...799..154M}
(11) \citet{2015ApJ...798L..13Y}; 
}
\end{deluxetable*}
...
\centering
	...
	from the IRSA Finderchart service (\url{http://irsa.ipac.caltech.edu/applications/finderchart/}).  Each image is centered on the target, and span a 1$\arcmin\times\arcmin$ field of view oriented with North up and East the left. The comparison stars referred to in the text appear as blue sources in these images. }
	\label{fig:images}
	\vspace{0.17in}
\end{figure*} 
...
\centering
	...
	(Bottom) Calculated brightness temperature spectra of {\sha} (left) and {\shb} (right). The faint lines represent individual calculations for a Monte Carlo realization of spectral flux, absolute magnitude, {\lbol} and age uncertainties; the dark lines represent the median of these realizations. For {\sha}, we show two  brightness temperature spectra corresponding to a young brown dwarf (50--200 Myr; blue) and a field dwarf (0.5--3~Gyr; black). We also show the brightness temperature spectra of the equivalent spectral model for the measured luminosity and assumed ages (dotted lines)  The corresponding {\teff} and {\logg} values are given in the legend.}
	\label{fig:tbright}
	\vspace{0.1in}
\end{figure*} 
...
\subfloat[2MASS J1821]{
	...
	\caption{(Top panels) Sine fit amplitude versus wavelength for each brown dwarf. \sha\ shows no clear trend in wavelength, while \shb\ has a strong slope indicative of Mie-scattering particles.
	 ...
	 {\it Bottom Panels:} The time of maximum variation $t_1$ for each brown dwarf shows no clear trend with wavelength.
	 For wavelengths with small variability amplitudes, $t_1$ becomes as wide as the prior window $-\tau/2 < t_1 < \tau/2$.
 [{\bf bottom panels need to be explained; what are y-axis units? why is t1 centered at 0? is this relative to some assumed peak time? what is that time? what about t2?}]}
	\label{fig:ampSpec}
\end{figure*} 
...
These variations are periodic with ``double-hump'' structures at peak amplitudes around -2.7~hr and +1.3~hr. {\bf [relative to what time?]}
...
{\bf [do you also constraint A1 $>$ A2?]}
...
{\bf [what is the value?]}
between the IRTF and WFC3 spectrum, presumably because they were taken at different epochs and therefore different spatial cloud and temperature distributions.
...
\begin{figure*}
\begin{centering}
\includegraphics[width=0.45\textwidth]{e_1821_096_map.png}\\
\includegraphics[width=0.45\textwidth]{e_1821_120_map.png}
\hspace{0.2in}
\includegraphics[width=0.45\textwidth]{e_1821_126_map.png}
\caption{{\it Aeolus} maps of the light curves at 0.96$\mu$m, 1.20$\mu$m and 1.26 $\mu$m.}\label{fig:spotModel}
\end{centering}
\end{figure*}
...
\section{Spot Distribution}
\newcommand{\maxSpots}{6}

{\bf AJB has some concerns over this section: it is extremely terse with no clear conclusions, and no discussion on the uniqueness of the spot solutions shown. The same temperature is used for each of the wavelengths, when the underlying brightness temperature actually changes; suggest using background temperature of for 1795 K for 0.96$\mu$m, 1788 K for 1.20$\mu$m, and 1853 K for 1.26$\mu$m based on the brightness temperature analysis. I would naively assume that the spot structures would be at the same location at each wavelength but vary in temperature offset - can a prior be applied so that each map has the same structure but different spot temperature (with Tspot = Tback as a possibility)?}

>>>>>>> remotes/origin/sharelatex-2017-06-21-0322
We also fit our light curves to an {\it Aeolus} spot model \citep{2015ApJ...814...65K,2016ApJ...825...90K}.
We allow up to \maxSpots\ {\bf [please fill in]} across the sphere of the brown dwarf, all with a constant temperature contrast.
We set the ``background'' at the measured effective temperature of 1614 K.

Figure \ref{fig:spotModel} shows the spot models fit to the light curve at 0.96$\mu$m, 1.20$\mu$m and 1.26 $\mu$m.
At all three wavelengths, the fitted brightness temperature contrast is around 50 K above the background.
These higher temperature spots are indicative of either holes in clouds or regions of elevated temperature.
The distribution of spots changes with wavelength, suggesting that the distribution of clouds or temperatures is complex.
The low amplitude and signal to noise of the light curves makes it difficult to put tight constraints on the spot distribution.
...
{\bf [need summary of AEOLUS results]}
