...
\textbf{
\subsubsection{Exploring the Hemispheric Assumption}
...
\end{equation}
In the optically thin case ($\tau \ll1$) the linear term dominates, so the amplitude, $A_{NH}$, is proportional to the optical depth $\tau$ and our assumption that $A = B \bar{Q_{avg}(r,\sigma_s)}$ is valid.
Only in the case where $\tau \gtrsim 1$ does the East-West hemispheric assumption break down. 
}
