...
\startdata
\sha\ & L5$^{(1)}$ & L7 & 7.27$\pm$0.02$^{(2)}$ & 11.94$\pm$0.02 & 2.03$\pm$0.03 & $-$4.05$\pm$0.08$^{(3)}$ & 1754$\pm$112$^{(3)}$ & 3.1\tablenotemark{b}$^{(4)}$ & 14.18$\pm$0.43$^{(5)}$ & 21 \\
...
\centering
	...
	For \sha\, we use the comparison star to the left and for \shb\ the lower right.}
	\label{fig:images}
	\vspace{0.17in}
\end{figure*} 
...
\subfloat[2MASS J1821]{
	...
	\caption{Top Panels: Normalized median raw spectra of {\sha} (left) and {\shb} (right) in black, compared with the respective reference stars (blue) and background emission (red; mostly telluric OH emission and thermal background). The areas of higher telluric absorption are visible near 1.4~$\mu$m and 1.8~$\mu$m.  Bottom Panels: Dynamic spectra of each brown dwarf system. The dynamic spectrum for \sha\ shows no significant variability in the spatial or spectral direction whereas \shb\ shows significant repeated modulations with a period near 4 hours.
...
The secondary amplitude $A_2$ shows a gradual decrease as a function of wavelength as with $A_1$, indicative of Mie scattering grains.
{\it Bottom Panel:} The time of maximum variation $t_2$ shows no clear phase shift trend.
...
As with Figure \ref{fig:ampSpec}, we find no strong phase shift for the second time of maximum variation $t_2$ as a function of wavelength.
...
This is different from  \citep{2016ApJ...826....8Y} who found a significant phase offset using simultaneous Spitzer and HST light curves at two different epochs.
This may be due to the different pressures probed by the Spitzer Channel's 1 and 2 (3.6 $\mu$m and 4.5 $\mu$m) not found in our wavelength range.
