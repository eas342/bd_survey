...
\centering
	...
	from the IRSA Finderchart service (\url{http://irsa.ipac.caltech.edu/applications/finderchart/}).  Each image is centered on the target, and span a 1$\arcmin\times$1$\arcmin$ field of view oriented with North up and East the left. The comparison stars referred to in the text appear as blue sources in these images.
	For \sha\, we use the comparison star to the left and for \shb\ to the lower right.}
	\label{fig:images}
	\vspace{0.17in}
\end{figure*} 
