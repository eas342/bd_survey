...
\begin{equation}\label{eq:2tempAdiff}
...
Figure \ref{fig:CMD} shows the color magnitude diagram from the sample of brown dwarfs in \citet{dupuy2012ltparallax}, which includes 2MASS J0835-0819 but not 2MASS J1812+1414.
...
\caption{Color-Magnitude diagram from \citet{dupuy2012ltparallax}. We added 2MASS J1821-0819 and 2MASS J0835+1414 using Simbad magnitudes and parallaxes.}\label{fig:CMD}
\end{centering}
\end{figure}
...
We impose priors on the amplitude $A_1$ so that it would be positive, $t_1$ would be less than the observation windows (to avoid multiple periodic solutions) and that the period $\tau$ would be between 0 and the observation window length.
...
\caption{Example posterior distributions for an MCMC fit to the time series using Equation \ref{eq:cosfit} for the wavelength bin 1.08~$\mu$m.}\label{fig:postCosfit}
\end{centering}
\end{figure}

We also explore a 2 term sinusoidal fit with a second cosine function at twice the frequency:
...
The $\chi^2$ per degree of freedom is still 7.9, so it is not the expected 1.0.
However, the error bars are likely underestimated when using photon and read noise limits.

The 2 term fit works better on the light curves that show the variations at a high signal to noise ratio
\begin{figure}
\begin{centering}
\includegraphics[width=0.5\textwidth]{best_fit_2term.pdf}
\caption{Best fit model to time series at 1.08~$\mu$m using Equation \ref{eq:cosfit2term}.}\label{fig:model2Cosfit}
\end{centering}
\end{figure}
...
\begin{figure}
\begin{centering}
\includegraphics[width=0.5\textwidth]{corner_fit_2term.pdf}
\caption{Example posterior distributions for an MCMC fit to the time series using Equation \ref{eq:cosfit} for the wavelength bin 1.08~$\mu$m.}\label{fig:post2Cosfit}
\end{centering}
\end{figure}
...
\caption{Maximum likelihood spectrum using an MCMC fit to the amplitude spectrum of 2MASS 1821.}\label{fig:ML2Tempfit}
\end{centering}
\end{figure}
...
\caption{Example posterior distributions for an MCMC fit to the spectra using Equation \ref{eq:2tempAdiff}.}\label{fig:post2Tempfit}
\end{centering}
\end{figure}
