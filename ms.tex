...
\begin{center}
\begin{tabular}{ccccr}
Name & Rotation Period & Spectral Type & K magnitude  & T$_\mathrm{eff}$ \\
-- & (hr) & -- & -- & (K) \\
\hline \hline
2MASS J1821+14010 & 4.2\tablenotemark{a} & L4 Pec\tablenotemark{b} - L5 Pec\tablenotemark{c} & 11.14 & 1600 $\pm$ 100\tablenotemark{b} \\
2MASS J0835-019 & 9 & L4 Pec\tablenotemark{b} - L5 & 11.65 & 1800 $\pm$ 100\tablenotemark{b} \\
\end{tabular}
\end{center}
...
Another simple model we attempt is a spot model - that the brown dwarf spectral variability has to do with a non-uniform temperature distribution on the brown dwarf in the same way that starspots modulate the brightness of hydrogen burning stars.
In Figure \ref{fig:ampspec1821tdiff}, we fit the same amplitude spectrum as in Figure \ref{fig:ampspec1821mie}.
This time we model the data with two layers in local thermodynamic equilibrium (LTE) so that the flux is a ratio of two Planck functions.
We include a multiplicative constant to account for the geometric area ratio of the two temperature layers.
The two-layer model best-fits the data with a reduced chi-squared of 4.0, which is significantly disfavored as compared to the Mie scattering mode.
Furthermore, the 400 K temperature variation in the model is significantly more than the expected 50 K horizontal temperature variations expected in circulation models \citep{showman2013bdgpDynamics}.

\begin{figure}
\begin{centering}
\includegraphics[width=0.5\textwidth]{amp_vs_wavl_j1821_t_diff.pdf}
\caption{The same data as in Figure \ref{fig:ampspec1821mie}. We fit the data as a temperature differential between the effective temperature of the brown dwarf and a cooler temperature.}\label{fig:ampspec1821tdiff}
\end{centering}
\end{figure}
