...
\startdata
\sha\ & L5$^{(1)}$ & L7 & 8.52$\pm$0.08$^{(3)}$ & 11.94$\pm$0.02 & 2.03$\pm$0.03 & $-$4.05$\pm$0.08$^{(3)}$ & 1754$\pm$112$^{(3)}$ & 3.1\tablenotemark{b}$^{(4)}$ & 14.18$\pm$0.43$^{(5)}$ & 21 \\
\shb\ & L4.5p & L5 & 9.38$\pm$0.03$^{(7)}$ & 12.40$\pm$0.02 & 1.78$\pm$0.03 & $-$4.18$\pm$0.03 & 1635$\pm$66 & 4.2$\pm$0.1 & 28.85$\pm$0.16$^{(5)}$ & 76 \\
\enddata
...
\end{deluxetable*}
%JKT:still don't know what these references go to; there aren't any numbers in the table.
...
The variability in \shb\ is prominent at the short wavelengths with a magnitude of ~$\sim$2\% peak-to-trough at short wavelengths but $\lesssim 0.5\%$ toward 2~$\mu$m.}
	...
	\vspace{0.1in}
\end{figure*} 
%JKT: I might add in this caption a note about the telluric features in both spectra at 1.4 and 1.8 $\mu$m, so people are clear about what they are looking at in this figure (without necessarily having to read through the text).
