...
If we assume the cloud covers one hemisphere of the brown dwarf, the amplitude of the variability is directly proportional to the opacity or $Q_{ext}$ of the particles.
We model the extinction coefficient, $Q_{ext}$ from the \texttt{Python} Mie theory code \texttt{miescatter} \citep{bohren1983mie} to calculate the extinction as a function of wavelength.
...
This model has a reduced $\chi^2$ of 0.99 and is thus consistent with the data.
Figure \ref{fig:corner1821mie} shows the posterior distribution for the $B$ and $r$ parameters.
The derived median particle radius of 0.164 $^{+0.012}_{-0.007}$~$\mu$m is a vary tight constraint for particle size median.
