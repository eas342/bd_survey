...
We imposed a prior that $A$ >= 0 so that a solution with a $\pi$ phase offset and negative amplitude isn't found.
...
\subfloat[2MASS J1821]{
	...
	\caption{Sine fit amplitude versus wavelength for each brown dwarf. The regions at 1.4~$\mu$m and 1.9~$\mu$m are contaminated by telluric absorption so the variations can likely be due to telluric variability not corrected by the reference star.}
	\label{fig:ampSpec}
\end{figure*} 
...
\citet{yang2015hstRotBD} also observe wavelength-dependent variability of 2MASS J1821.
