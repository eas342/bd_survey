...
\subfloat[2MASS J1821]{
	...
	\caption{Sine fit amplitude versus wavelength for each brown dwarf. \sha\ shows no clear trend in wavelength, while \shb\ has a strong slope indicative of Mie-scattering particles.
	 ...
	 \sha\ is consistent with a zero amplitude flat amplitude model which essentially puts no constraints on the particle size distribution.
...
\begin{split}
F(t) = A_1 \cos(2 \pi (t - t_1)/\tau) + A_2 \cos(4 \pi (t - t_2)/\tau) + \\
B t + C + D a(t)
\end{split}
\end{equation}
where $F(t)$ is the flux as a function of time $t$, $A_1$ and $A_2$ are the amplitudes of the two terms, $t_1$ and $t_2$ is the time of maximum periodic flux for each cosine term, $\tau$ is period of the variations, and $B$, $C$ and $D$ are terms describing the baseline trend and $a(t)$ is the airmass as a function of time.
...
If we assume the cloud covers one longitudinal hemisphere of the brown dwarf, the amplitude of the variability ($A$) is directly proportional to the opacity of the particles.
...
When fitting the amplitude spectra to our Mie model, we impose a prior on the maximum median particle size radius ($0 \mu m < r < 5 \mu m$).

For \sha\ we find that since no significant variability is detected that the particle size is unconstrained over the prior of $0 \mu < r < 5 \mu m$).
We show the maximum likelihood particle radius of 2.3 $\mu$m
...
2MASS J1821 shows significant variability up to the 1.3\% amplitude (2.6\% peak-to-valley) at short wavelengths (0.9$\mu$m) which decreases towards the near infrared (2.4$\mu$m).
