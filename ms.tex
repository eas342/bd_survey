...
\begin{abstract}
L and T brown dwarfs show variability associated with the rotation of heterogeneous atmospheres. We present a high precision ground-based time series study of 2 brown dwarfs - 2MASS J08354256-0819237 and 2MASS J18212815+1414010, both of spectral type L4-L5 using the Infrared Telescope Facility (IRTF). 2MASS J0835 shows low level ($\lesssim 0.4\%$) spectral variability with no wavelength dependence from 0.8~$\mu$m to 2.4~$\mu$m. 2MASS J1821, by contrast, shows $\sim$2\%  variability in the shorter wavelengths that falls off toward 2.4~$\mu$m. The variability amplitude spectrum can be fit by Mie extinction of sub-micron dust particles. It can also be fit by a hot spots on the brown dwarf surface, but they require implausible temperature variations, so we prefer the Mie scattering model.
\end{abstract}
...
Brown dwarfs can also be observed over their rotation periods to understand the spatial cloud structure and composition, that can change rapidly with time \citep[e.g.][]{yang2016exStormsBD}.
...
\citet{burgaser2014irtf} first demonstrated that the Infrared Telescope Facility (IRTF) is a useful tool for characterizing brown dwarf variability and that a simultaneous reference can be used to correct for common-mode variability, such as telluric transmission variations.
...
We observed the two brown dwarfs as part of a pilot program to assess the efficacy of IRTF for measuring spectral variability in brown dwarf atmospheres with the exoplanet techniques of \citet{schlawin2014} to achieve high precision spectro-photometry.
...
2MASS J08354256-0819237 \citep{cruz2003coolNeighbors} is also an L4-L5 brown dwarf known to be variable with a tentative period of 3.1 hr \citep{koen2004ibandBD}, which also permitted about 1.5 rotation periods.
...
We fit the variability amplitude spectra with two different physical models - Mie scattering from clouds and a heterogenous temperature distribution.
...
We use the same method as \citet{schlawin2014} to obtain high precision spectrophotometry which employs a large 3\arcsec $\times$ 60\arcsec\ slit.
We used a 0.9~$\mu$m dichroic to split the optical wavelengths to the MORIS imager \citep{Gulbis2011}.
For guiding, we use the MORIS imager in the SDSS $i$ band.
For 2MASS J0835, we used short (<15 sec) integrations as is standard for guiding, so the source was too faint for photometry in $i$ band.
For 2MASS J1821, we extended the integration time to 60 seconds in order to obtain simultaneous $i$ band photometry.

\begin{table*}
\begin{center}
\caption{Observing Parameters}\label{tab:obsParam}
\begin{tabular}{cccccc}
Target & Observing Date & N$_{int}$ & t$_{int}$ & Duration & Reference Star \\
-- & UT Start & -- & (s) & (hr) &  -- \\
\hline
2MASS J1821 & 2015-12-31 & 363 & 45 & 4.65 &  2MASS 18212622+1414064 \\
2MASS J0835 & 2016-06-25 & 352 & 60 & 7.47 &  2MASS 08354383-0819212 \\
\end{tabular}
\end{center}
\tablenotetext{}{N$_{int}$ is the number of integrations and t$_{int}$ is the effective integration time for the 32 non-destructive reads up the ramp. The duration is the total time from start to end of the observations including the reset times and overheads.}
\end{table*}
...
2MASS J0835 exhibits very little variability over our 4.7 hr whereas 2MASS 1821 shows large variability at the 4.2 hr rotation period, shown in Figure \ref{fig:tserPfold}.
...
\end{equation}
where $F$ is the flux as a function of time ($t$), A is the amplitude of the variations, $t_0$ is the time offset of the variations, $\tau$ is period of the variations and B and C are linear terms describing the baseline trend.
For the fits, we let $A$, $t_0$, $B$ and $C$ be free parameters.
We fixed the sinusoidal period $\tau$ at the literature value of rotation period (Table \ref{tab:bdProp}) because there are trends (with a period of $\sim$24 hr) due to uncorrected telluric variations that that are not relevant to the brown dwarf system.
...
Therefore, we suspect that hot or cold spots on the surface are a less plausible explanation of the variability of 2MASS 1821 than a heterogenous temperature distribution.
...
\caption{Color-Magnitude diagram from \citet{dupuy2012ltparallax}. We added 2MASS J1821 and 2MASS J0835 using Simbad magnitudes and parallaxes.}\label{fig:CMD}
\end{centering}
\end{figure}
