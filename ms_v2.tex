...
\colhead{Conditions} \\
 & &  &  & & \colhead{(s)} & \colhead{(hr)} & \colhead{Range} }
\startdata
{\sha} & 2016-06-25 & XXX & XXX & 352 & 60 & 7.47 & XXX-XXX & J08354383-0819212 & XXX \\
{\shb} & 2015-12-31 & XXX & XXX & 363 & 45 & 4.65 & XXX-XXX & J18212622+1414064 & XXX \\
\enddata
\tablecomments{N$_{int}$ is the number of integrations and t$_{int}$ is the effective integration time for the 32 non-destructive reads up the ramp. The duration is the total time from start to end of the observations including the reset times and overheads.}
\end{deluxetable*}
...
\subfloat[2MASS J1821]{
	...
	\caption{Time series for each system with an offset added for clarity. Each wavelength bin is labeled on the right side. Alternating colors are used to distinguished the light curves with offsets. {\bf [increased vertical scale so fluctuations are visible; replace diamonds with vertical bars (unc) or smaller symbols]}}
	\label{fig:tserPfold}
\end{figure*} 
...
We extracted separate lightcurves for each target in 25 spectral bins of width XXX~$\mu$m, as shown in Figure~\ref{fig:tserPfold}. Each of these were fit to a simple sinusoidal model {\bf [NOTE: CHANGE TO COSINE FOR SIMPLER INTERPRETATION OF TO]:
\begin{equation}
...
This is the same trend observed for 2MASS J1821 by HST over a shorter wavelength range (1.1 to 1.7~$\mu$m) by \citet{yang2016exStormsBD}.
